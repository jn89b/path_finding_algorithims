...
if __name__ == '__main__':
    ...
    obstacle_list = [(17,5), (5,17)]
    obstacle_list = add_obstacles(grid, obstacle_list)
    
    landing_zones = [(10,10), (7,7), (15,15)]
    plot_landing_zones(landing_zones)
    
    # #uav0
    uav_0 = UAV("uav0", [1,1], landing_zones[0])
    new_obstacle = obstacle_list + return_other_zones(landing_zones[:], 0)
    grid_copy = grid.copy()
    new_obstacle = add_obstacles(grid_copy, new_obstacle)
    astar = Astar(grid_copy, new_obstacle,  uav_0.starting_position, uav_0.goalpoint)
    uav_0.path = astar.main()
    plot_path(grid_row, grid_col,uav_0.path, new_obstacle, uav_0.goalpoint)
    
    # # #uav1
    uav_1 = UAV("uav1", [10,0], landing_zones[1])
    new_obstacle = obstacle_list + return_other_zones(landing_zones[:], 1) + uav_0.path 
    new_obstacle = add_obstacles(grid, new_obstacle)
    grid_copy = grid.copy()
    astar = Astar(grid, new_obstacle,  uav_1.starting_position, uav_1.goalpoint)
    uav_1.path = astar.main()
    plot_path(grid_row, grid_col,uav_1.path, new_obstacle , uav_1.goalpoint)
    
    # #uav 1
    # new_obstacle = obstacle_list 
    # new_obstacle = new_obstacle + uav_0.path 
    # new_obstacle = add_obstacles(grid, new_obstacle)
    # uav_1 = UAV("uav1", [3,0], landing_zones[1])
    # astar_2 = Astar(grid, new_obstacle, uav_1.starting_position, uav_1.goalpoint)
    # uav_1.path = astar_2.main()
    # plot_path(grid_row, grid_col,uav_1.path, new_obstacle, uav_1.goalpoint)
    
    #uav 2
    # new_obstacle = new_obstacle + uav_1.path
    # new_obstacle = add_obstacles(grid, new_obstacle)
    # uav_2 = UAV("uav2", [20,0], landing_zones[2])
    # astar_2 = Astar(grid, new_obstacle, uav_2.starting_position, uav_2.goalpoint)
    # uav_2.path = astar_2.main()
    # plot_path(grid_row, grid_col,uav_2.path, new_obstacle, uav_2.goalpoint)
    
    #uav 3
    # new_obstacle = new_obstacle + uav_2.path
    # new_obstacle = add_obstacles(grid, new_obstacle)
    # uav_3 = UAV("uav3", [10,0], landing_zones[3])
    # astar_3 = Astar(grid, new_obstacle, uav_3.starting_position, uav_3.goalpoint)
    # uav_3.path = astar_3.main()
    # plot_path(grid_row, grid_col,uav_3.path, new_obstacle, uav_3.goalpoint)
